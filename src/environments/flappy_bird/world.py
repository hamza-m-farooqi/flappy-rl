...
from src.config import load_env_game_config, normalize_game_mode
from src.environments.flappy_bird.bird import Bird
from src.environments.flappy_bird.pickup import Pickup
from src.environments.flappy_bird.pipe import Pipe
...
class World:
    ...
    def from_config(cls, population_size: int = 1, mode: str = "easy") -> World:
        """Create a world using the environment-specific configuration file."""
        return cls(
            config=load_env_game_config("flappy_bird"),
            mode=mode,
